...
contract SimpleBond is ISimpleBond, Ownable {
  ...
  uint8[] sustainabilityMeasurements;
   ...
   constructor(string _name, uint256 _par, uint256 _parDecimals, uint256 _coupon,
               ...
               uint256 _loopLimit, address _oracle) {
  ...
  function updateTotalDebt(uint256 variablePayment) public {
    ...
    // totalDebt = totalDebt + (variablePayment + (totalsBonds * parValue * 0.03))
    sustainabilityMeasurements.push(variablePayment);
    intervalCount += 1;

    emit TotalDebtUpdated(totalDebt)
