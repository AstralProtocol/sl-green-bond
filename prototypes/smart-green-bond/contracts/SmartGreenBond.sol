...
contract SmartGreenBond is ISimpleBond, Ownable {
    ...
    function updateTotalOwed(uint256 noxMeasurement) public onlyOracle { // called every 1 year by an off-chain oracle EOA
        ...
        // hold for now // require(intervalCount.add(1).mul(couponThreshold) < block.number, 'An update to the variable rate is not yet due.');
        ...
        // ?? Not used - for now. Keeps track of which payment interval we're on.
        // intervalCount += 1;
